...
Committed env templates lived at the repository root even though they were operational examples rather than authority surfaces. Temporary generated env files also defaulted into the repository root, which made preserved artifacts and failed cleanup paths clutter the bootstrap surface. The merge-gate workflow still used an older `actions/checkout` major and lacked run concurrency controls and a job timeout.

## Decision

The repository root stays centered on authority and execution surfaces. Committed env templates move under `ops/env/`, and temporary generated env files move under `.cache/env/`.
...
- Temporary generated env artifacts no longer compete with repository entrypoints.
