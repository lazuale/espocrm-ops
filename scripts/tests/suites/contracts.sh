...
test_toolkit_layer_boundaries() {
  ...
  if ! run_command_capture "$output_file" bash -lc "
    ...
    ! rg -Fq 'source \"\$SCRIPT_DIR/lib/fs.sh\"' '$SCRIPT_DIR/restore-db.sh'
    rg -Fq 'run_espops \"\${args[@]}\"' '$SCRIPT_DIR/restore-db.sh'
    rg -Fq 'restore --scope' '$SCRIPT_DIR/restore-db.sh'
    ! rg -Fq 'wait_for_service_ready db' '$SCRIPT_DIR/restore-db.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/backup.sh\"' '$SCRIPT_DIR/restore-db.sh'
    ! rg -Fq 'compose up -d' '$SCRIPT_DIR/restore-db.sh'
    ! rg -Fq 'compose stop' '$SCRIPT_DIR/restore-db.sh'
    ...
    rg -Fq 'run_espops \"\${args[@]}\"' '$SCRIPT_DIR/restore-files.sh'
    rg -Fq 'restore --scope' '$SCRIPT_DIR/restore-files.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/backup.sh\"' '$SCRIPT_DIR/restore-files.sh'
    ! rg -Fq 'compose up -d' '$SCRIPT_DIR/restore-files.sh'
    ! rg -Fq 'compose stop' '$SCRIPT_DIR/restore-files.sh'
    ...
    rg -Fq 'run_espops \"\${args[@]}\"' '$SCRIPT_DIR/restore-drill.sh'
    rg -Fq 'restore-drill' '$SCRIPT_DIR/restore-drill.sh'
    ! rg -Fq 'verify-backup' '$SCRIPT_DIR/restore-drill.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/restore-db.sh\"' '$SCRIPT_DIR/restore-drill.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/restore-files.sh\"' '$SCRIPT_DIR/restore-drill.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/status-report.sh\"' '$SCRIPT_DIR/restore-drill.sh'
    ! rg -Fq 'compose up -d' '$SCRIPT_DIR/restore-drill.sh'
    ! rg -Fq 'compose down' '$SCRIPT_DIR/restore-drill.sh'

    rg -Fq 'source \"\$SCRIPT_DIR/lib/common.sh\"' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'source \"\$SCRIPT_DIR/lib/locks.sh\"' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'source \"\$SCRIPT_DIR/lib/compose.sh\"' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'source \"\$SCRIPT_DIR/lib/artifacts.sh\"' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'source \"\$SCRIPT_DIR/lib/fs.sh\"' '$SCRIPT_DIR/support-bundle.sh'
    rg -Fq 'run_espops \"\${args[@]}\"' '$SCRIPT_DIR/support-bundle.sh'
    rg -Fq 'support-bundle' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'acquire_operation_lock support-bundle' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'resolve_env_file' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'load_env' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'ensure_runtime_dirs' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'compose config' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'compose ps' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'compose logs' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'docker version' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'docker compose version' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/doctor.sh\"' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/status-report.sh\"' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/backup-catalog.sh\"' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'cleanup_old_files' '$SCRIPT_DIR/support-bundle.sh'
    ! rg -Fq 'tar -C ' '$SCRIPT_DIR/support-bundle.sh'
    ...
    rg -Fq 'run_espops \"\${args[@]}\"' '$SCRIPT_DIR/rollback.sh'
    rg -Fq 'rollback' '$SCRIPT_DIR/rollback.sh'
    ! rg -Fq 'acquire_operation_lock rollback' '$SCRIPT_DIR/rollback.sh'
    ! rg -Fq 'resolve_env_file' '$SCRIPT_DIR/rollback.sh'
    ! rg -Fq 'load_env' '$SCRIPT_DIR/rollback.sh'
    ! rg -Fq 'ensure_runtime_dirs' '$SCRIPT_DIR/rollback.sh'
    ! rg -Fq 'acquire_maintenance_lock rollback' '$SCRIPT_DIR/rollback.sh'
    ! rg -Fq 'run-operation' '$SCRIPT_DIR/rollback.sh'
    ! rg -Fq 'rollback-plan' '$SCRIPT_DIR/rollback.sh'
    ! rg -Fq 'verify-backup' '$SCRIPT_DIR/rollback.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/status-report.sh\"' '$SCRIPT_DIR/rollback.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/backup.sh\" \"\$ESPO_ENV\"' '$SCRIPT_DIR/rollback.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/restore-db.sh\"' '$SCRIPT_DIR/rollback.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/restore-files.sh\"' '$SCRIPT_DIR/rollback.sh'
    ! rg -Fq 'compose up -d' '$SCRIPT_DIR/rollback.sh'
    ! rg -Fq 'wait_for_service_ready_with_shared_timeout' '$SCRIPT_DIR/rollback.sh'
    ! rg -Fq 'wait_for_application_stack_with_shared_timeout' '$SCRIPT_DIR/rollback.sh'
    ! rg -Fq 'http_probe \"\$SITE_URL\"' '$SCRIPT_DIR/rollback.sh'
    ...
    rg -Fq 'run_espops \"\${args[@]}\"' '$SCRIPT_DIR/migrate-backup.sh'
    rg -Fq 'migrate-backup' '$SCRIPT_DIR/migrate-backup.sh'
    ! rg -Fq 'acquire_operation_lock migrate-backup' '$SCRIPT_DIR/migrate-backup.sh'
    ! rg -Fq 'resolve_env_file' '$SCRIPT_DIR/migrate-backup.sh'
    ! rg -Fq 'load_env' '$SCRIPT_DIR/migrate-backup.sh'
    ! rg -Fq 'ensure_runtime_dirs' '$SCRIPT_DIR/migrate-backup.sh'
    ! rg -Fq 'acquire_maintenance_lock' '$SCRIPT_DIR/migrate-backup.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/restore-db.sh\"' '$SCRIPT_DIR/migrate-backup.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/restore-files.sh\"' '$SCRIPT_DIR/migrate-backup.sh'
    ! rg -Fq 'compose up -d' '$SCRIPT_DIR/migrate-backup.sh'
    ! rg -Fq 'compose stop' '$SCRIPT_DIR/migrate-backup.sh'
    ...
    rg -Fq 'run_espops \"\${args[@]}\"' '$SCRIPT_DIR/contour-overview.sh'
    rg -Fq 'overview' '$SCRIPT_DIR/contour-overview.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/doctor.sh\"' '$SCRIPT_DIR/contour-overview.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/status-report.sh\"' '$SCRIPT_DIR/contour-overview.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/backup-audit.sh\"' '$SCRIPT_DIR/contour-overview.sh'
    ! rg -Fq 'run_repo_script \"\$SCRIPT_DIR/backup-catalog.sh\"' '$SCRIPT_DIR/contour-overview.sh'
    ! rg -Fq 'add_overview_section' '$SCRIPT_DIR/contour-overview.sh'
    ...
    rg -Fq 'run_espops \"\${args[@]}\"' '$SCRIPT_DIR/status-report.sh'
    rg -Fq 'status-report' '$SCRIPT_DIR/status-report.sh'
    ! rg -Fq 'acquire_operation_lock status-report' '$SCRIPT_DIR/status-report.sh'
    ! rg -Fq 'resolve_env_file' '$SCRIPT_DIR/status-report.sh'
    ! rg -Fq 'load_env' '$SCRIPT_DIR/status-report.sh'
    ! rg -Fq 'ensure_runtime_dirs' '$SCRIPT_DIR/status-report.sh'
    ! rg -Fq 'service_status_or_default() {' '$SCRIPT_DIR/status-report.sh'
    ! rg -Fq 'render_lock_entries_json() {' '$SCRIPT_DIR/status-report.sh'
    ! rg -Fq 'latest_backup_file' '$SCRIPT_DIR/status-report.sh'
    ...
    ! rg -Fq 'create_failure_bundle() {' '$SCRIPT_DIR/support-bundle.sh'
