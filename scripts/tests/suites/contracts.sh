...
test_toolkit_layer_boundaries() {
  ...
  if ! run_command_capture "$output_file" bash -lc "
    ...
    rg -Fq 'DB_STORAGE_DIR=../espocrm-data/runtime/dev/db' '$ROOT_DIR/.env.dev.example'
    rg -Fq 'ESPO_STORAGE_DIR=../espocrm-data/runtime/dev/espo' '$ROOT_DIR/.env.dev.example'
    rg -Fq 'BACKUP_ROOT=../espocrm-data/backups/dev' '$ROOT_DIR/.env.dev.example'
    rg -Fq 'DB_STORAGE_DIR=../espocrm-data/runtime/prod/db' '$ROOT_DIR/.env.prod.example'
    rg -Fq 'ESPO_STORAGE_DIR=../espocrm-data/runtime/prod/espo' '$ROOT_DIR/.env.prod.example'
    rg -Fq 'BACKUP_ROOT=../espocrm-data/backups/prod' '$ROOT_DIR/.env.prod.example'
    rg -Fq '.env.smoke.*' '$ROOT_DIR/.gitignore'
    rg -Fq '.env.restore-drill.*' '$ROOT_DIR/.gitignore'
...
test_ci_guards_repo_local_artifacts() {
  ...
  if ! run_command_capture "$output_file" bash -lc "
    ...
    rg -Fq 'actions/checkout@v5' '$ROOT_DIR/.github/workflows/ai-governance.yml'
    ...
    rg -Fq 'make ci' '$ROOT_DIR/.github/workflows/ai-governance.yml'

    rg -Fq 'ai-refresh:' '$ROOT_DIR/Makefile'
    rg -Fq 'ai-check:' '$ROOT_DIR/Makefile'
    rg -Fq 'ci: ai-check test vet ai-shell-json-smoke bashcheck shellcheck' '$ROOT_DIR/Makefile'
