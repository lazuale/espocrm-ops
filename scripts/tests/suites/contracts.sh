...
test_toolkit_layer_boundaries() {
  ...
  if ! run_command_capture "$output_file" bash -lc "
    ...
    rg -Fq 'run_espops \"\${update_backup_args[@]}\"' '$SCRIPT_DIR/update.sh'
    rg -Fq 'run_espops \"\${update_runtime_args[@]}\"' '$SCRIPT_DIR/update.sh'
...
test_ci_guards_repo_local_artifacts() {
  ...
  if ! run_command_capture "$output_file" bash -lc "
    ...
    rg -Fq 'timeout-minutes: 30' '$ROOT_DIR/.github/workflows/ai-governance.yml'
