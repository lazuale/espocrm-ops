...
func runtimePrepareSkippedDetails(req ExecuteRequest) string {
...
func runtimeReturnSkippedDetails(req ExecuteRequest, prep runtimePrepareInfo) string {
	switch {
	case req.NoStop:
		return "Application services remained running because of --no-stop."
...
func notRunBackupStep(code, summary string) ExecuteStep {
	return ExecuteStep{
		Code:    code,
		Status:  BackupStepStatusNotRun,
		Summary: summary,
	}
}
