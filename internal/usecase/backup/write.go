...
func FinalizeBackup(req FinalizeRequest) (FinalizeInfo, error) {
	...
	if err := WriteSHA256Sidecar(req.DBBackupPath, manifest.Checksums.DBBackup, dbSidecarPath); err != nil {
		return FinalizeInfo{}, fmt.Errorf("write db sha256 sidecar: %w", err)
	}
	if err := WriteSHA256Sidecar(req.FilesBackupPath, manifest.Checksums.FilesBackup, filesSidecarPath); err != nil {
		return FinalizeInfo{}, fmt.Errorf("write files sha256 sidecar: %w", err)
	}
	if err := WriteManifest(req.ManifestPath, manifest); err != nil {
		return FinalizeInfo{}, err
	}
...
func BuildManifest(req ManifestBuildRequest) (domainbackup.Manifest, error) {
	...
	dbChecksum, err := platformfs.SHA256File(req.DBBackupPath)
	if err != nil {
		return domainbackup.Manifest{}, fmt.Errorf("hash db backup: %w", err)
	}
	filesChecksum, err := platformfs.SHA256File(req.FilesBackupPath)
	if err != nil {
...
func WriteManifest(path string, manifest domainbackup.Manifest) error {
	return backupstore.WriteManifest(path, manifest)
}

func WriteSHA256Sidecar(filePath, checksum, sidecarPath string) error {
	return backupstore.WriteSHA256Sidecar(filePath, checksum, sidecarPath)
}
